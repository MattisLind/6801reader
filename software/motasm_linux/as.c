...
char	**argv;
{
	...
	if(argc < 2){
		printf("Usage: asx file1 file2 [-opt1 opt2]\n");
		printf("Options:\n");
		printf("    l	   enable output listing.\n");
		printf("    nol    disable output listing (default).\n");
		printf("    cre    generate cross reference table.\n");
		printf("    s	   generate a symbol table.\n");
		printf("    c	   enable cycle count.\n");
		printf("    noc    disable cycle count.\n");
		exit(1);
		}
	  Argv = argv;
	  initialize();
